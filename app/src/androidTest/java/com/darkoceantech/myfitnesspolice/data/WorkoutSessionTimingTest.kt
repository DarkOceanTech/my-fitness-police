...
class WorkoutSessionTimingTest {
    ...
    @Test fun finalCooldownFinishesAtOneMinuteAndTimersResumeAfterProcessDeath() = runBlocking {
        val context = InstrumentationRegistry.getInstrumentation().targetContext
        ...
        try {
            ...
            time = 7000
            assertTrue(runCatching { progress.endRest("w") }.isFailure)
            assertEquals("cooldown", db.sessionStateDao().get("w")!!.phase)
            ...
            assertEquals(60000L, db.workoutSetDao().getForExercise("we").single().restMillis)
            assertEquals(64000L, db.sessionStateDao().get("w")!!.dutyElapsedMillis)
            assertEquals(65000L, db.workoutDao().getDetails("w")!!.workout.finishedAt)
        } finally { db.close() }
    }
}
