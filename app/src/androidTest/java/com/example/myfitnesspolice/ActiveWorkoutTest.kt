...
class ActiveWorkoutTest {
    ...
    @Test fun carouselActualRepsPauseResumeAndPartialFinishKeepPlanIntact() {
        ...
        try {
            ...
            compose.onNodeWithContentDescription("Actual reps ${sets[0].id}").performClick()
            compose.onNodeWithTag("Actual reps ${sets[0].id} options").performScrollToNode(hasText("7"))
            compose.onNodeWithText("7", substring = false).performClick()
            compose.waitUntil(5000) { runBlocking { repo.observeWorkout(id).first()!!.orderedSets()[0].actualReps == 7 } }
            runBlocking { assertNull(repo.observeWorkout(id).first()!!.orderedSets()[0].completedAt) }
            ...
            compose.onNodeWithTag("actual-reps-input").assert(SemanticsMatcher.expectValue(
                SemanticsProperties.EditableText, androidx.compose.ui.text.AnnotatedString("7")))
